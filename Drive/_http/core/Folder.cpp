...
std::vector<Document*> Folder::get_contents() {
	return contents;
}

bool Folder::is_folder() {
	return true;
}

std::string Folder::info_http() {
    std::string info;
    info += name + " | ";
    info += "folder | ";
...
Document* Folder::subset_folder(std::string subset_name) {
	...
	for(int i=0; i<contents.size(); i++)
		if(contents[i]->is_same_folder(subset_name))
			return contents[i];
	throw Server::Exception("there is no such directory: "+subset_name);
}
...
Document* Folder::select_file(std::string file_name) {
	for(int i=0; i<contents.size(); i++)
		if(contents[i]->is_same_file(file_name))
			return contents[i];
	throw Server::Exception("there is no such file: "+file_name);
}
...
void Folder::remove(std::string doc_name) {
	...
	throw Server::Exception("there is no such file: "+doc_name);
}
